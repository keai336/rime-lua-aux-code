...
2. 本插件需附加至特定输入方案。首先，复制你所需使用的输入方案文件名，将文件名中的 `schema` 改为 `custom`。然后，创建并打开一个名为 `*.custom.yaml` 的文件，在其中添加所需内容：
    ...
    patch:
      engine/filters/+:
        - lua_filter@*aux_code@fly_moqi@true@;@s       #@方案名@提示与否(true or false)@引导键@匹配模式(s 严格  l宽松)    每一项都要配置,不能空
        # 或下面的小鹤形码方案
...
若要制作个人码表，确保文件格式为 UTF-8 编码即可。文件中每一行应对应一个字的`字 音 形`，使用 `制表符` 号作为分隔符。若同一汉字有多种编码方案，应分别在新的一行中列出，如：
